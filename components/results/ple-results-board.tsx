...
function MatchResultRow({
  ...
  onSaved: (board: PleBoard) => void;
}) {
  const existing = matchRow.result;
  const [pick, setPick] = useState<DraftPick | null>(() => {
    if (!existing) return null;
    if (matchCard.format === "multi" && existing.winnerIndex != null) {
      const name =
        matchCard.competitors[existing.winnerIndex]?.name ??
        existing.winnerName ??
        "";
      return { kind: "multi", index: existing.winnerIndex, name };
    }
    if (existing.winnerSide === "left" || existing.winnerSide === "right") {
      const name =
        existing.winnerSide === "left"
      ...
      return {
        kind: "singles",
        side: existing.winnerSide,
        name: existing.winnerName ?? name,
      };
    }
    return null;
  });
  ...
  const register = async () => {
    ...
    setError(null);
    try {
      const board = await submitPleMatchResult(slug, matchRow.id, {
        ...draftFromMatch(matchRow, pick),
        status: "finished",
      });
      onSaved(board);
      setSaved(true);
  ...
  const hasResult = saved || !!matchRow.result;
  ...
  return (
    ...
    >
          ...
          matchCard.competitors.map((c, idx) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() =>
                  setPick({ kind: "multi", index: idx, name: c.name })
                }
                className={cn(
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        >
          {saving ? "등록 중…" : hasResult ? "결과 다시 등록" : "결과 등록"}
        </button>
        {error && (
      ...
      {!loading && (
        <ul className="space-y-4">
          {matches.map((matchRow) => {
            const matchCard = boardMatchesToCards([matchRow])[0]!;
            return (
              <MatchResultRow
                key={matchRow.id}
                slug={slug}
